# Operator variables
# ==================
export OPERATOR_NAME=compliance-profile-operator
OPERATOR_IMAGE_NAME=manager
PROFILEPARSERBIN=profileparser
PROFILEPARSER_IMAGE_NAME=$(PROFILEPARSERBIN)

# Container image variables
# =========================
IMAGE_REPO?=quay.io/compliance-profile-operator
RUNTIME?=podman
...
OPERATOR_IMAGE_PATH?=$(IMAGE_REPO)/$(OPERATOR_IMAGE_NAME)
PROFILEPARSER_IMAGE_PATH=$(IMAGE_REPO)/$(PROFILEPARSER_IMAGE_NAME)
PROFILEPARSER_DOCKERFILE_PATH=./images/profileparser/Dockerfile
...
TARGET=$(TARGET_DIR)/bin/$(OPERATOR_NAME)
PROFILEPARSER_TARGET=$(TARGET_DIR)/bin/$(PROFILEPARSERBIN)
...
export NAMESPACE?=openshift-compliance-profile-operator
...
COURIER_PACKAGE_NAME=$(OPERATOR_NAME)-bundle
COURIER_OPERATOR_DIR=deploy/olm-catalog/$(OPERATOR_NAME)
COURIER_QUAY_NAMESPACE=$(OPERATOR_NAME)
COURIER_PACKAGE_VERSION?=
OLD_COURIER_PACKAGE_VERSION=$(shell find deploy/olm-catalog/$(OPERATOR_NAME)/ -mindepth 1 -maxdepth 1 -type d -printf '%f\n' | sort -r | head -1)
COURIER_QUAY_TOKEN?= $(shell cat ~/.quay)
PACKAGE_CHANNEL?=alpha

.PHONY: all
all: build ## Test and Build the compliance-profile-operator
...
.PHONY: profileparser-image
profileparser-image:
	$(RUNTIME) build -f $(PROFILEPARSER_DOCKERFILE_PATH) -t $(PROFILEPARSER_IMAGE_PATH):$(TAG) .

.PHONY: build
build: fmt manager profileparser ## Build the compliance-profile-operator binary

manager:
	$(GO) build -o $(TARGET) cmd/manager/main.go

profileparser:
	$(GO) build -o $(PROFILEPARSER_TARGET) cmd/profileparser/main.go
...
.PHONY: run
run: operator-sdk ## Run the compliance-profile-operator locally
...
.PHONY: test-unit
test-unit: fmt ## Run the unit tests (not implemented yet)
ifndef JUNITFILE
...
e2e-local: operator-sdk ## Run the end-to-end tests on a locally running operator (e.g. using make run) (not implemented yet)
	@echo "WARNING: This will temporarily modify deploy/operator.yaml"
	@echo "Replacing workload references in deploy/operator.yaml"
	@sed -i 's%$(IMAGE_REPO)/$(PROFILEPARSER_IMAGE_NAME):latest%$(PROFILEPARSER_IMAGE_PATH)%' deploy/operator.yaml
...
# probably want to push the compliance-profile-operator image to the cluster we're
...
image-to-cluster:
	...
	$(eval component = $(OPERATOR_NAME))
	$(eval OPERATOR_IMAGE_PATH = $(IMAGE_FORMAT))
	$(eval component = compliance-profileparser)
	$(eval PROFILEPARSER_IMAGE_PATH = $(IMAGE_FORMAT))
...
image-to-cluster: namespace openshift-user image  ## Push the local images to the cluster
	...
	IMAGE_REGISTRY_HOST=$$(oc get route default-route -n openshift-image-registry --template='{{ .spec.host }}'); \
		...
		$(RUNTIME) push --tls-verify=false $(OPERATOR_IMAGE_PATH):$(TAG) $${IMAGE_REGISTRY_HOST}/$(NAMESPACE)/$(OPERATOR_IMAGE_NAME):$(TAG); \
		$(RUNTIME) push --tls-verify=false $(PROFILEPARSER_IMAGE_PATH):$(TAG) $${IMAGE_REGISTRY_HOST}/$(NAMESPACE)/$(PROFILEPARSER_IMAGE_NAME):$(TAG);
...
.PHONY: push
push: image
	# compliance-profile-operator manager
	$(RUNTIME) push $(OPERATOR_IMAGE_PATH):$(TAG)
	# profileparser
	$(RUNTIME) push $(PROFILEPARSER_IMAGE_PATH):$(TAG)
...
.PHONY: csv
csv: deploy/olm-catalog/$(OPERATOR_NAME)/$(COURIER_PACKAGE_VERSION) check-package-version operator-sdk ## Generate the CSV and packaging for the specific version (NOTE: Gotta specify the version with the COURIER_PACKAGE_VERSION environment variable)

deploy/olm-catalog/$(OPERATOR_NAME)/$(COURIER_PACKAGE_VERSION):
...
release-tag-image: package-version-to-tag
	...
	@sed -i 's%$(IMAGE_REPO)/$(OPERATOR_IMAGE_NAME):latest%$(OPERATOR_IMAGE_PATH):$(TAG)%' deploy/operator.yaml
	@sed -i 's%$(IMAGE_REPO)/$(PROFILEPARSER_IMAGE_NAME):latest%$(PROFILEPARSER_IMAGE_PATH):$(TAG)%' deploy/operator.yaml

.PHONY: undo-deploy-tag-image
undo-deploy-tag-image: package-version-to-tag
	@echo "Restoring image tags in deploy/operator.yaml"
	@sed -i 's%$(OPERATOR_IMAGE_PATH):$(TAG)%$(IMAGE_REPO)/$(OPERATOR_IMAGE_NAME):latest%' deploy/operator.yaml
	@sed -i 's%$(PROFILEPARSER_IMAGE_PATH):$(TAG)%$(IMAGE_REPO)/$(PROFILEPARSER_IMAGE_NAME):latest%' deploy/operator.yaml

.PHONY: git-release
git-release: package-version-to-tag
	git checkout -b "release-v$(TAG)"
	git add "deploy/olm-catalog/$(OPERATOR_NAME)/$(TAG)"
	git add "deploy/olm-catalog/$(OPERATOR_NAME)/$(OPERATOR_NAME).package.yaml"
